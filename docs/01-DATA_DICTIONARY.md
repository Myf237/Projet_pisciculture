...
3. **Ammoniac :** valeurs extrêmes allant jusqu'à 4,27 × 10¹¹, alors que la plage réaliste en aquaculture est de l'ordre de 0–1 mg/L. **Tranché par ADR-003 (2026-09-18) : seuillé à 5 (27 560 valeurs non manquantes marquées artefact, soit 33,18 %), les 66,82 % restants conservés.** Distribution réelle du sous-ensemble > 5 (correction du 2026-09-18, mesure `qa-validator`, `reports/validations/jalon-1.md`) : **1 838 valeurs distinctes**, de 5,00082 à 4,27 × 10¹¹, médiane 127,87, seulement **25 relevés** exactement à la valeur maximale et 3 847 (13,96 %) entre 5 et 10 — un **continuum de valeurs implausibles pour un bassin viable**, pas une constante répétée façon code d'erreur (contrairement à ce qu'affirmait la première rédaction de l'ADR-003, corrigée). Seuil maintenu à 5 par décision humaine reconfirmée sur ces chiffres (J-20260918-028) : au-delà, aucune valeur n'est compatible avec un bassin viable, et la variable est de toute façon utilisée en indicateur relatif (capteur de gaz, ADR-009). À documenter explicitement dans le mémoire comme limite du dataset brut.
...
8. **Oxygène dissous irréaliste (A2) — TRANCHÉ (ADR-010, 2026-09-18) :** maximum observé 41,046 mg/L. Les seuils du cahier §5 restent applicables (sonde immergée, ADR-009). Borne physique supérieure : **15 mg/L, définitive** (confirmée par l'humain le 2026-09-18), 21 614 relevés (26,00 % du fichier) marqués hors borne ; alternatives écartées : 8 mg/L (45,48 % du fichier au-delà) et 20 mg/L (21,41 %) — chiffres : `reports/analyse-donnees-jalon1.md`, §2. **Complément (ADR-011, Jalon 2, 2026-09-18) :** cette borne rendait invisible dans le fichier nettoyé l'intégralité de l'épisode du plateau brut 36-41 mg/L (30/07-05/08, 13 422 relevés) — trou de ~6 jours, au-delà de `MAX_INTERPOLATION_GAP` (1 h). La borne reste appliquée à la colonne nettoyée, mais la valeur brute d'origine est désormais conservée dans une colonne parallèle `Dissolved Oxygen_raw` (suffixe `config.RAW_VALUE_SUFFIX`), jamais imputée : 368 valeurs seulement dans la colonne nettoyée sur cet épisode, contre 13 422 dans la colonne brute, moyenne 36,47 mg/L (`reports/validations/jalon-1.md`, J-20260918-039/044). **Cadrage de ces chiffres (précision du 2026-09-19, défaut D11, `reports/validations/jalon-2.md` §7) :** fenêtre **fermée ligne à ligne** [2021-07-30 02:00:00, 2021-08-05 09:00:00] — moyenne pondérée exacte 36,4668 mg/L (arrondie ici à 36,47). Sur la **couverture des 152 créneaux horaires** utilisée par `resample_hourly` (Jalon 2), dont le dernier (09:00 du 05/08) n'est pas fermé, la fenêtre inclut 25 relevés de plus : 13 447 valeurs brutes présentes, moyenne pondérée exacte 36,4071 mg/L, ou 36,1569 mg/L pour la moyenne des 140 moyennes horaires non-NaN (non pondérée) — trois valeurs exactes, chacune dans son cadrage ; aucune ne corrige les autres. Conséquence pour l'exploitation : le Jalon 3 doit choisir explicitement entre la colonne nettoyée (analyse physiologique, seuils applicables) et la colonne `_raw` (détection de dérive de capteur, démonstration au Jalon 5).
...
*Anomalies 12 et 13 ajoutées le 2026-09-18 lors de l'exploration du Jalon 2 (constat data-engineer J-20260918-039, décisions G1 J-20260918-042, ADR-011).*

12. **pH sous 4 (Jalon 2) — TRANCHÉ (ADR-011, 2026-09-18) :** 145 relevés de pH strictement inférieurs à 4, dans la plage de nettoyage [0, 14] (donc visibles, non marqués manquants). Répartition corrigée après vérification — un brief de délégation affirmait à tort qu'ils étaient tous situés dans la fenêtre du 24/09 au 01/10 (erreur constatée et corrigée, J-20260918-040) : en réalité **62 relevés du 15 au 19/09** et **83 relevés du 11 au 13/10**, deux fenêtres disjointes de celle du 24/09-01/10 (qui contient, elle, les 40 relevés de pH négatif, déjà hors bornes [0,14] et marqués manquants — voir anomalie 2). Décision : aucun resserrement de `PH_BOUNDS` (reste [0, 14]) ; ces 145 relevés restent visibles comme anomalies de capteur, un filtrage plus strict restant possible au Jalon 3.
13. **Turbidité saturée (Jalon 2) — TRANCHÉ (ADR-011, 2026-09-18) :** 56,37 % des relevés de `Turbidity(NTU)` sont au plafond observé de 100 NTU (`reports/analyse-donnees-jalon1.md`, §6 et addendum Jalon 2) — une limite du capteur plutôt qu'un état réel du bac. Décision : aucun seuil absolu, variable traitée en **indicateur relatif** (même raisonnement que l'ammoniac et le nitrate, ADR-009) ; un seuil absolu déclencherait une alerte sur plus de la moitié du cycle, non exploitable pour la démonstration.
...
- [x] Fixer le seuil de turbidité (« à définir après exploration », cahier des charges §5) — ADR-011 (2026-09-18) : indicateur relatif, aucun seuil absolu (saturation à 100 NTU sur 56,37 % des relevés, anomalie 13)
