...
> **Dernière mise à jour :** 2026-09-18 — doc-keeper (G2 PR #2 fusionnée, G3 replanification ADR-008, Jalon 1 démarré, J-20260918-013)
...
- **Jalon actif :** **Jalon 1** (Données nettoyées et fiables) — analyse factuelle des données par le data-engineer en cours (A1, A2, ADR-003) ; Jalon 2 (Exploration) à mener le même jour, juste après.
...
1. **Jalon 1, en cours** : `data-engineer` analyse factuellement les colonnes concernées par A1, A2 et ADR-003 (empreinte déjà vérifiée, voir `docs/01`), sur la branche `feat/jalon-1-donnees-nettoyees` — décisions à trancher (**G1**) avant d'écrire les règles de nettoyage, puis implémentation de `ingestion.py` et vérification du Jalon 1.
