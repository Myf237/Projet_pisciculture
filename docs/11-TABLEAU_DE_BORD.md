...
> **Dernière mise à jour :** 2026-09-19 — doc-keeper (Jalon 2 VALIDÉ AVEC RÉSERVES, itération 2 — `reports/validations/jalon-2.md` — 3 critères conformes, 9 défauts recensés dont **8 levés** (D1-D8) et **D9 partiellement levé** (ligne de corps à 73 caractères dans `194db31`, non corrigeable sans réécrire un historique déjà poussé) ; **3 défauts nouveaux en itération 2** : **D10 levé** (6 tests de cas limite ajoutés, `pytest` 105 réussis / 27 ignorés / 0 échec), **D11 levé** (chiffres de l'épisode 1 réconciliés, J-20260919-013), **D12 traité par ADR-012** (Accepté, épinglage de l'environnement + périmètre des empreintes du Jalon 6, J-20260919-014/016) ; G1 du 2026-09-19 : signal brut propagé en aval, J-20260919-001 ; corrections D3/D4 dans `docs/03`)
...
- **Jalon actif :** **Jalon 1** (Données nettoyées et fiables) clos — **VALIDÉ** (5 critères `docs/04` conformes, réserves toutes levées, `reports/validations/jalon-1.md` itérations 1-2), fusionné (PR #3, merge commit `700bb26`) et **tagué `jalon-1`**. **Jalon 2** (Exploration validée) **VALIDÉ AVEC RÉSERVES, itération 2** (`reports/validations/jalon-2.md`, J-20260919-008) : les 3 critères `docs/04` conformes, sans régression ; **8 défauts sur 9 de l'itération 1 levés** (D1-D8 — D1, seule réserve à portée technique, tranchée le 2026-09-19 par décision **G1**, propagation du signal brut en aval, J-20260919-001), **D9 partiellement levé** (écart de forme résiduel, non corrigeable sans réécrire un historique déjà poussé) ; **3 défauts nouveaux constatés en itération 2** : **D10 levé** (data-engineer, 6 tests de cas limite ajoutés, J-20260919-012), **D11 levé** (doc-keeper, réconciliation documentaire, J-20260919-013), **D12** (figures commitées non régénérables par l'environnement documenté) **traité par ADR-012, Accepté** le 2026-09-19 (épinglage de l'environnement en versions exactes, périmètre des empreintes du Jalon 6 limité aux sorties de données — décision humaine G1/G3, J-20260919-014, consignée J-20260919-016).
- **Avancement :** replanifié (ADR-008) après un jour de retard sur l'ADR-006 (mise en place non comptée comme jalon le 2026-09-17) ; plus aucune marge de planning avant la démonstration du 2026-09-23 (voir `08-REGISTRE_RISQUES.md`, R1). Constat majeur du Jalon 2 (la borne DO rendait l'épisode 1 invisible dans le fichier nettoyé) traité par ADR-011 sans rouvrir l'ADR-010 ; sa propagation en aval tranchée le 2026-09-19 (J-20260919-001). Contre-vérification indépendante de l'itération 2 (J-20260919-008) : dérive non tracée de l'environnement logiciel détectée (matplotlib 3.10.8 → 3.11.2 en cours de Jalon 2, défaut D12) et diagnostic initial corrigé (J-20260919-010) ; tranchée par ADR-012 (épinglage `requirements.txt`/`requirements-lock.txt`, nouveau risque R17, `08-REGISTRE_RISQUES.md`).
...
| 2 | Exploration validée | J1 — 18/09 | data-engineer | Validé avec réserves, itération 2 (3 critères conformes, 8/9 défauts levés, D9 partiel, D10-D11 levés, D12 traité par ADR-012) | 2026-09-19 | `reports/validations/jalon-2.md` |
...
1. **Jalon 2, VALIDÉ AVEC RÉSERVES (itération 2)** (`reports/validations/jalon-2.md`) : D1-D8 levés, D9 partiellement levé (écart de forme résiduel, sans action possible sans réécrire un historique poussé), D10 et D11 levés, D12 traité par ADR-012 (Accepté) — plus aucune réserve à portée technique ouverte. Reste à faire avant la PR : régénération des 8 figures `reports/figures/jalon2_*.png` depuis le venv épinglé (data-engineer, en cours en parallèle de cet ADR). Puis **pull request du Jalon 2** et décision **G2** de fusion.
